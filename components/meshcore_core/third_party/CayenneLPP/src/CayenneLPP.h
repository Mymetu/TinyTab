...
#endif
// Native frameworks only need ArduinoJson when JSON decoding is explicitly enabled.
#if !defined(ARDUINO) && defined(CAYENNE_ENABLE_ARDUINO_JSON)
#include <ArduinoJson.h>
#endif
...
// Arduino, or native frameworks that explicitly enable ArduinoJson decoding.
#if defined(ARDUINO) || defined(CAYENNE_ENABLE_ARDUINO_JSON)
