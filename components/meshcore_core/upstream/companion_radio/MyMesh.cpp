...
void MyMesh::onContactOverwrite(const uint8_t* pub_key) {
  ...
  meshcore_port_on_contact_removed(pub_key);
...
bool MyMesh::onContactLoaded(const ContactInfo &contact) {
  if (!addContact(contact)) return false;

  const bool route_known = contact.out_path_len != OUT_PATH_UNKNOWN &&
                           mesh::Packet::isValidPathLen(contact.out_path_len);
  const uint8_t hop_count = route_known ? (contact.out_path_len & 0x3F) : 0;
  meshcore_port_on_contact_loaded(contact.id.pub_key, contact.name, contact.type,
                                  route_known, hop_count);
  return true;
}

void MyMesh::onDiscoveredContact(ContactInfo &contact, bool is_new, uint8_t path_len, const uint8_t* path) {
  const bool route_known = path != nullptr && mesh::Packet::isValidPathLen(path_len);
  const uint8_t hop_count = route_known ? (path_len & 0x3F) : 0;
  meshcore_port_on_contact_advert(contact.id.pub_key, contact.name, contact.type,
                                  route_known, hop_count,
                                  static_cast<int16_t>(radio_driver.getLastRSSI()),
                                  static_cast<int16_t>(radio_driver.getLastSNR() * 4.0f));
...
void MyMesh::onMessageRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
                           const char *text) {
  meshcore_port_on_contact_message(from.id.pub_key);
...
void MyMesh::sendFloodAdvert(mesh::Packet* pkt) {
  if (!pkt) return;
  TransportKey scope;
...
void MyMesh::handleCmdFrame(size_t len) {
  if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
    ...
    if (recipient) {
      ...
      const bool route_known = recipient->out_path_len != OUT_PATH_UNKNOWN &&
                               mesh::Packet::isValidPathLen(recipient->out_path_len);
      meshcore_port_on_contact_loaded(recipient->id.pub_key, recipient->name,
                                      recipient->type, route_known,
                                      route_known ? (recipient->out_path_len & 0x3F) : 0);
      dirty_contacts_expiry = futureMillis(LAZY_CONTACTS_WRITE_DELAY);
      writeOKFrame();
    } else {
      ContactInfo contact;
      ...
      if (addContact(contact)) {
        ...
        meshcore_port_on_contact_loaded(contact.id.pub_key, contact.name, contact.type,
                                        route_known,
                                        route_known ? (contact.out_path_len & 0x3F) : 0);
        dirty_contacts_expiry = futureMillis(LAZY_CONTACTS_WRITE_DELAY);
        writeOKFrame();
      } else {
        writeErrFrame(ERR_CODE_TABLE_FULL);
