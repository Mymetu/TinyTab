...
static const uint32_t REMOTE_GPS_HISTORY_MAGIC = 0x31534752; // "RGS1"
static const uint16_t REMOTE_GPS_HISTORY_FORMAT_VERSION = 1;

uint32_t RemoteGpsHistoryStore::calculateCrc32(const uint8_t* data, size_t length) {
...
void RemoteGpsHistoryStore::makeSegmentPath(uint8_t slot, char* path, size_t path_size) {
  snprintf(path, path_size, "/rgps%02u.bin", slot);
}

bool RemoteGpsHistoryStore::scanSegment(uint8_t slot, SegmentInfo& info) {
  ...
  uint32_t header_crc = calculateCrc32((const uint8_t*)&header,
                                       sizeof(header) - sizeof(header.crc32));
  if (header.magic != REMOTE_GPS_HISTORY_MAGIC ||
      header.version != REMOTE_GPS_HISTORY_FORMAT_VERSION ||
  ...
  while (info.count < RECORDS_PER_SEGMENT &&
    ...
    uint32_t record_crc = calculateCrc32((const uint8_t*)&disk_record,
                                         sizeof(disk_record) - sizeof(disk_record.crc32));
...
void RemoteGpsHistoryStore::updateBounds() {
  uint32_t cursor = _next_sequence;
  uint32_t count = 0;

  while (count < REMOTE_GPS_HISTORY_MAX_RECORDS) {
    int8_t preceding_slot = -1;
    for (uint8_t slot = 0; slot < REMOTE_GPS_HISTORY_SEGMENT_COUNT; slot++) {
      ...
      if (preceding_slot < 0 || info.generation > _segments[preceding_slot].generation) {
        preceding_slot = slot;
      }
    }
  ...
  if (count > REMOTE_GPS_HISTORY_MAX_RECORDS) count = REMOTE_GPS_HISTORY_MAX_RECORDS;
  _record_count = count;
  _oldest_sequence = _next_sequence - count;
}

bool RemoteGpsHistoryStore::begin(fs::FS* filesystem, uint32_t new_session_id) {
  _fs = filesystem;
  _ready = false;
  _current_slot = -1;
  _session_id = 0;
...
bool RemoteGpsHistoryStore::createNextSegment() {
  uint8_t target_slot = _current_slot < 0 ? 0 : (_current_slot + 1) % REMOTE_GPS_HISTORY_SEGMENT_COUNT;
  ...
  SegmentHeader header;
  header.magic = REMOTE_GPS_HISTORY_MAGIC;
  header.version = REMOTE_GPS_HISTORY_FORMAT_VERSION;
  header.header_size = sizeof(SegmentHeader);
...
bool RemoteGpsHistoryStore::append(const RemoteGpsHistoryRecord& record) {
  if (!_ready) return false;

  if (_current_slot < 0 || !_segments[_current_slot].appendable ||
  ...
  disk_record.crc32 = calculateCrc32((const uint8_t*)&disk_record,
                                     sizeof(disk_record) - sizeof(disk_record.crc32));
  ...
  _segments[_current_slot].appendable = _segments[_current_slot].count < RECORDS_PER_SEGMENT;
  _next_sequence++;
  updateBounds();
  return true;
}

bool RemoteGpsHistoryStore::read(uint32_t sequence, RemoteGpsHistoryRecord& record) {
  ...
  for (uint8_t slot = 0; slot < REMOTE_GPS_HISTORY_SEGMENT_COUNT; slot++) {
    ...
    uint32_t crc = calculateCrc32((const uint8_t*)&disk_record,
                                  sizeof(disk_record) - sizeof(disk_record.crc32));
